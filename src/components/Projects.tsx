...
const Projects = () => {
  ...
  return (
    <section id="projects" className="px-4 sm:px-6 md:px-8 py-16 md:py-28 relative">
        ...
        <div className="space-y-6 md:space-y-8">
          {projects.map((project, i) => (
            <motion.div
              ...
              className={`glass-card p-5 sm:p-6 md:p-8 relative overflow-hidden group transition-all duration-500 hover:shadow-neon ${
                project.featured
            ...
            >
              ...
              <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-3 md:mb-4">
                ...
                {project.tag && (
                  <span className="px-2 sm:px-3 py-1 text-[10px] sm:text-xs font-medium rounded-full bg-neon-purple/20 text-neon-purple border border-neon-purple/30">
                    {project.tag}
                  </span>
                )}
                {project.featured && (
                  <span className="px-2 sm:px-3 py-1 text-[10px] sm:text-xs font-medium rounded-full bg-neon-blue/20 text-neon-blue border border-neon-blue/30">
                    Featured
                  </span>
                )}
              </div>
              ...
              <div className="grid sm:grid-cols-2 gap-4 md:gap-6 mb-4 md:mb-6">
                <div>
                  <h4 className="text-xs md:text-sm font-semibold text-neon-blue mb-2 md:mb-3 uppercase tracking-wider">Key Features</h4>
                  <ul className="space-y-1.5 md:space-y-2">
                    {project.features.map((f) => (
                      <li key={f} className="text-xs md:text-sm text-muted-foreground flex items-start gap-2">
                ...
                <div>
                  <h4 className="text-xs md:text-sm font-semibold text-neon-purple mb-2 md:mb-3 uppercase tracking-wider">Tech Stack</h4>
                  <div className="flex flex-wrap gap-1.5 md:gap-2">
                    {project.tech.map((t) => (
                      <span key={t} className="px-2 md:px-3 py-1 text-[10px] md:text-xs rounded-lg bg-muted/50 border border-glass-border text-foreground">
                        {t}
                      </span>
                    ))}
                  </div>
              ...
              <div className="flex gap-4">
                {project.github && (
                  <button
                    ...
                    className="btn-outline-glow flex items-center gap-2 text-xs md:text-sm !px-3 md:!px-4 !py-2"
                  >
